...
function openWhatsapp(e) {
  e.preventDefault();
  const user_name = jQuery('#form-field-name').val();
  const no_guests = jQuery('#form-field-noTickets').val();
  const confirmation = document.querySelector('input[name="form_fields[confirmation]"]:checked').value;
  const wish = jQuery('#form-field-message').val();
  let full_message = ""

  if (confirmation === "Asistiré"){
    full_message = encodeURIComponent(`Hola, soy ${user_name}. ${confirmation} con ${no_guests} y este es mi deseo ${wish}.`)
  }else{
    full_message = encodeURIComponent(`Hola, soy ${user_name}. ${confirmation} y este es mi deseo ${wish}.`)
  }
  const url = `https://wa.me/525538855001/?text=${full_message}`
